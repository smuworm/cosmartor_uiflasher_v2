...
class ZlgCanBus(BusABC):
    ...
    def __init__(
        ...
        print(self._dev_info[interface]["dev_type"])
   
        # 打开设备
        self._dev_handle = self.device.OpenDevice(self._dev_info[interface]["dev_type"], dev_index, 0)
        #set usbcan-e-u baudrate
        if self._dev_info[interface]["dev_type"] in (USBCAN_XE_U_TYPE + USBCAN_I_II_TYPE + USBCANFD_TYPE):
            print(str(channel))
            ip = self.device.GetIProperty(self._dev_handle)
            self.device.SetValue(ip, 
                                str(channel) + "/baud_rate", 
                                self._dev_info[interface]["chn_info"]["baudrate"][bitrate])
            self.device.ReleaseIProperty(ip)

        chn_cfg = driver.ZCAN_CHANNEL_INIT_CONFIG()
        # 是否是CANFD设备
        if self._dev_info[interface]["chn_info"]["is_canfd"]:
            chn_cfg.can_type = driver.ZCAN_TYPE_CANFD
        else:
            chn_cfg.can_type = driver.ZCAN_TYPE_CAN
        # 0表示正常模式，能收能发
        chn_cfg.config.can.mode = 0
        if self._dev_info[interface]["dev_type"] in USBCAN_I_II_TYPE:
        ...
        for channel in self.channels:
            ...
            self._chn_handle[channel] = self.device.InitCAN(self._dev_handle, channel, chn_cfg)
            # 初始通道结束后，启动CAN通道
    ...
    def poll_received_messages(self) -> None:
        ...
        for channel in self.channels:
            ...
            zcan_raw_msgs, act_rcv_num = self.device.Receive(
                self._chn_handle[channel], rcv_num)
            # type: list
    ...
    def send(self, msg: Message, transmit_type: int = 0, timeout: Optional[float] = None) -> None:
    ...
    def shutdown(self) -> None:
        super().shutdown()
        for channel in self.channels:
            res = self.device.ResetCAN(self._chn_handle[channel])
